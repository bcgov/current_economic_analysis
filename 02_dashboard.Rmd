...
plotly_ts <- function(df, thing, format_as, tt_text, theme, type, pal) {
  # df <- df%>%
  #   group_by(Series)%>%
  #   mutate(label = if_else(Month == max(Month), as.character(Series), NA_character_))
  ...
  #   expand_limits(x =tsibble::yearmonth(lubridate::today()+lubridate::years(3)))+
  #   geom_text(aes(label = label), hjust = 0, nudge_x = 500)
...
plotly::renderPlotly({
  ...
  bindCache(
    input$name, input$theme, input$type,
    input$pal
  )
```
