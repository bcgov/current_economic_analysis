...
long <- readRDS(here::here("processed_data", "long.rds"))
...
commentary_file <- here::here("commentary", "commentary.md")
```
...
- This dashboard provides time series pertinent to British Columbia's current economic outlook.
...
*  Two data normalizations are considered:
    -  Binning: values $v$ are replaced by their (within series) percentile score.
    -  Rescaling: values are replaced by $(v-\min(v))/(\max(v)-\min(v))$


```{r}
selectInput("norm",
  label = "Choose a data normalization:",
  choices = c("Binning","Rescaling"),
  selected = "Rescaling"
)
...
renderPlot({
 pheatmap::pheatmap(wide(),
         cluster_cols=FALSE,
         angle_col=0,
         main = str_to_title(paste("Monthly data, normalized by", input$norm)),
